...
def eval_visual(eval_loader, model, device, path, visual_dir):
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_loader):
            ...
            fig, axs = plt.subplots(n_latents + 1, n_traversal, constrained_layout=True)
            axs = trim_axs(axs, len(images))
            for i, (ax, image) in enumerate(zip(axs, images)):
                image = np.squeeze(np.moveaxis(image.numpy(), 0, 2))
                ax.imshow(image, cmap=cm.get_cmap('gray') if len(image.shape) == 2 else None)
                ax.set_axis_off()
            fig.savefig(os.path.join(visual_dir, 'step-%d-image-%d.png' % (step, i)))
            plt.close(fig)
