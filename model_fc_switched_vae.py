...
def conditional_entropy_upper_bound(y_logits, y_logits_2):
    ...
    return (- log_p.exp() * log_p_2).sum(1).mean()
...
class EncSwitchedFC(nn.Module):
    ...
    def forward(self, x):
        ...
        ctrl = self.switch(out).reshape(-1, 3, N_BRANCHES)  # B x 3 x n_branches
...
class DecSwitchedFC(nn.Module):
    ...
    def forward(self, x, y_index, y_hard, z):
        out = F.relu(x)  # B x n_dims
...
class ConvEncoder(nn.Module):
    def __init__(self, in_channels):
        ...
        self.switches = nn.ModuleList([EncSwitchedFC(1024) for _ in range(N_SWITCHES)])
...
class DeconvDecoder(nn.Module):
    def __init__(self, out_channels):
        ...
        self.switches = nn.ModuleList([DecSwitchedFC(1024) for _ in range(N_SWITCHES)])
...
class FCSwitchedVAE(nn.Module):
    def __init__(self, y_ce_beta, y_phsic_beta, y_mmd_beta, z_beta, z2_beta, img_channels):
        ...
        self.decoder = DeconvDecoder(img_channels)
    ...
    def loss(self, x, recon_x, z2_mean, z2_logvar, ys_logits, ys_logits_2, ys_hard, zs_mean, zs_logvar):
        ...
        for y_hard in ys_hard:
            y_mmd_loss += categorical_mmd_from_uniform(y_hard, N_BRANCHES)
